...
export default function App() {
  ...
  const loadListings = useCallback(async () => {
    try {
      const idx = await storage.get("listing-index", true);
      const ids = idx?.value ? JSON.parse(idx.value) : [];
      const list = [];
      for (const id of ids) {
        try {
          const r = await storage.get(`listing:${id}`, true);
          if (r?.value) list.push(JSON.parse(r.value));
        } catch (e) {}
      }
      setListings(list);
  ...
  const addListing = async (listing) => {
    const idxR = await storage.get("listing-index", true).catch(() => null);
    const ids = idxR?.value ? JSON.parse(idxR.value) : [];
    await storage.set("listing-index", JSON.stringify([...ids, listing.id]), true);
    await storage.set(`listing:${listing.id}`, JSON.stringify(listing), true);
    await loadListings();
  };
  ...
  const deleteListing = async (listing) => {
    const idxR = await storage.get("listing-index", true).catch(() => null);
    const ids = idxR?.value ? JSON.parse(idxR.value) : [];
    await storage.set("listing-index", JSON.stringify(ids.filter((id) => id !== listing.id)), true);
    await storage.delete(`listing:${listing.id}`, true);
    await loadListings();
  };
  ...
  useEffect(() => {
    ...
    loadReports();
    loadBoostHistory();
  }, [loadUsers, loadListings, loadCategories, loadSuggestions]);
  ...
  const toggleBoostListing = async (l) => {
    // Bascule manuelle par l'admin (gratuite, sans expiration) — utile pour un geste commercial.
    const next = l.boosted
      ? { ...l, boosted: false, boostedAt: null, boostExpiresAt: null }
      : { ...l, boosted: true, boostedAt: Date.now(), boostExpiresAt: null, boostRequest: null };
    await updateListing(next);
  };

  const requestBoostListing = async (l, days) => {
    const offer = BOOST_OFFERS.find((o) => o.days === days);
    if (!offer) return;
    const reference = `BST-${l.id.slice(0, 4).toUpperCase()}-${Date.now().toString().slice(-5)}`;
    await updateListing({
      ...l,
  ...
  const cancelBoostRequestListing = async (l) => {
    await updateListing({ ...l, boostRequest: null });
  };

  const confirmBoostListing = async (l) => {
    if (!l.boostRequest) return;
    const expiresAt = Date.now() + l.boostRequest.days * dayMs;
    await logBoostHistory({ listingId: l.id, listingTitle: l.title, userName: l.userName, days: l.boostRequest.days, amount: l.boostRequest.amount, reference: l.boostRequest.reference, confirmedAt: Date.now() });
    await updateListing({ ...l, boosted: true, boostedAt: Date.now(), boostExpiresAt: expiresAt, boostRequest: null });
  };

  const rejectBoostRequestListing = async (l) => {
    await updateListing({ ...l, boostRequest: null });
  };

  const updateListingRating = async (l, avgRating, ratingCount) => {
    await updateListing({ ...l, avgRating, ratingCount });
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.paper, fontFamily: "'Inter', -apple-system, sans-serif", color: C.ink }}>
      ...
      <main style={{ maxWidth: 980, margin: "0 auto", padding: "0 20px 80px" }}>
        {view.name === "home" && <Home listings={listings} categories={categories} loading={loading} onNav={setView} q={q} onSuggest={async (text) => { await addSuggestion(text); flash("Merci ! Votre suggestion a été transmise."); }} />}
        {view.name === "category" && <CategoryList category={view.category} listings={listings} categories={categories} onNav={setView} />}
        {view.name === "profile" && (
          <Profile
            user={users.find((u) => u.id === view.id)}
            listings={listings.filter((l) => l.userId === view.id)}
            me={me}
        ...
        {view.name === "new-listing" && (
          <ListingForm
            me={me}
            categories={categories}
            existing={view.editId ? listings.find((l) => l.id === view.editId) : null}
            onSave={async (listing) => {
              ...
              setView({ name: "listing", id: listing.id });
            }}
            onNav={setView}
          />
        )}
        {view.name === "listing" && (
          <ListingDetail
            listing={listings.find((l) => l.id === view.id)}
            me={me}
            categories={categories}
            onNav={setView}
            flash={flash}
            onDelete={async (l) => { await deleteListing(l); flash("Annonce supprimée."); setView({ name: "profile", id: l.userId }); }}
            onRequestBoost={requestBoostListing}
            onCancelBoostRequest={cancelBoostRequestListing}
            onUpdateRating={updateListingRating}
            onReport={async (listingId, title, reason) => { await addReport(listingId, title, reason); flash("Signalement envoyé — merci."); }}
          />
        )}
        {view.name === "signup" && <Signup me={me} categories={categories} onCreated={async (u) => { await addUser(u); }} onNav={setView} />}
        ...
        {view.name === "admin" && (
          <Admin
            ...
            onToggleBoost={toggleBoostListing}
            onConfirmBoost={async (l) => { await confirmBoostListing(l); flash("Mise en avant activée."); }}
            onRejectBoostRequest={async (l) => { await rejectBoostRequestListing(l); flash("Demande refusée."); }}
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "12px 20px", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", flexShrink: 0 }} onClick={() => onNav({ name: "home" })}>
          <div style={{ width: 32, height: 32, background: C.amber, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 6 }}>
            <span className="disp" style={{ fontSize: 14, color: "#000" }}>AB</span>
          </div>
          <span className="disp" style={{ fontSize: 19, color: C.ink }}>Bénin Services<span style={{ color: C.amber }}>+</span></span>
        </div>
        ...
        <nav className="sans" style={{ display: "flex", alignItems: "center", gap: 14, fontSize: 14, marginLeft: "auto" }}>
          {me ? (
            <>
              ...
              {me.status === "approved" && (
                <button className="btn" onClick={() => onNav({ name: "new-listing" })} style={{ background: C.amber, color: "#fff", padding: "10px 18px", borderRadius: 20, fontWeight: 700, whiteSpace: "nowrap" }}>
                  + Publier
                </button>
              )}
              <button className="btn" onClick={onLogout} style={{ background: "transparent", color: "#5A6270", display: "flex", alignItems: "center", gap: 5, padding: 0, fontSize: 13 }}>
...
function Hero() {
  return (
    <section className="sans" style={{ background: C.amber, padding: "22px 20px", borderRadius: 12, color: "#fff", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
      <div>
        <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.06em", opacity: 0.85, marginBottom: 4 }}>LE CARNET D'ADRESSES DES BONS PRESTATAIRES</div>
...
function Home({ listings, categories, loading, onNav, onSuggest, q }) {
  const live = sortListings(listings);
  const query = (q || "").trim().toLowerCase();
  const results = query
    ? live.filter((l) => l.title?.toLowerCase().includes(query) || l.city?.toLowerCase().includes(query) || l.userName?.toLowerCase().includes(query) || l.description?.toLowerCase().includes(query))
    : live.slice(0, 8);
      ...
      <section style={{ padding: "26px 0" }}>
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px,1fr))", gap: 14 }}>
            {results.map((l) => (
              <ListingCard key={l.id} l={l} categories={categories} onNav={onNav} />
            ))}
          </div>
        )}
      </section>
...
function EmptyState({ onNav }) {
  return (
    <div className="sans card" style={{ padding: 28, borderRadius: 10, textAlign: "center", color: "#5A6270" }}>
      <p style={{ margin: "0 0 14px" }}>Aucune annonce pour l'instant. Soyez le premier prestataire du carnet.</p>
...
function ListingCard({ l, categories, onNav }) {
  const cat = categories.find((c) => c.id === l.category);
  return (
    <button className="btn card" onClick={() => onNav({ name: "listing", id: l.id })} style={{ textAlign: "left", padding: 0, borderRadius: 10, overflow: "hidden", position: "relative", boxShadow: isBoostActive(l) ? `0 0 0 2px ${C.amber}` : "0 1px 2px rgba(0,0,0,.04)" }}>
      <div style={{ position: "relative", aspectRatio: "4/3", background: C.paperDeep }}>
        {l.photos?.[0] ? (
          <img src={l.photos[0]} alt={l.title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
        ) : (
      ...
      <div style={{ padding: "10px 12px 12px" }}>
        <div className="sans" style={{ fontSize: 14, fontWeight: 700, color: C.ink, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{l.title}</div>
        <div className="sans" style={{ fontSize: 12, color: C.amberDeep, fontWeight: 600, marginTop: 3 }}>{cat?.label}</div>
        <div className="sans" style={{ fontSize: 12, color: "#8A909B", marginTop: 2 }}>{l.city}</div>
        {l.ratingCount > 0 && <div style={{ marginTop: 5 }}><Stars rating={l.avgRating} count={l.ratingCount} /></div>}
      </div>
    </button>
  );
}
...
function CategoryList({ category, listings, categories, onNav }) {
  const [q, setQ] = useState("");
  const cat = categories.find((c) => c.id === category);
  const base = sortListings(listings.filter((l) => l.category === category));
  const query = q.trim().toLowerCase();
  const list = query ? base.filter((l) => l.title?.toLowerCase().includes(query) || l.city?.toLowerCase().includes(query)) : base;
  return (
    <section style={{ padding: "30px 0" }}>
      ...
      <div className="sans" style={{ position: "relative", marginBottom: 20 }}>
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Filtrer par ville ou par titre…" style={{ ...inputStyle, width: "100%", padding: "13px 14px 13px 40px", fontSize: 14 }} />
      </div>
      {list.length === 0 ? (
        query ? <p className="sans" style={{ color: "#8A909B" }}>Aucun résultat pour cette recherche.</p> : <EmptyState onNav={onNav} />
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px,1fr))", gap: 14 }}>
          {list.map((l) => <ListingCard key={l.id} l={l} categories={categories} onNav={onNav} />)}
        </div>
      )}
    </section>
  );
...
function Profile({ user, listings, me, categories, onNav, flash, onUpdateUser, onReport }) {
    ...
    <section style={{ padding: "30px 0" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginTop: 16, flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
          <Avatar photo={null} name={user.name} size={64} />
          <div>
            <h2 className="disp" style={{ fontSize: 26, margin: 0 }}>{user.name}</h2>
      ...
      {user.status === "pending" && isOwner && (
        <div className="sans card" style={{ padding: 14, borderRadius: 10, marginTop: 18, fontSize: 13, color: "#5A6270", borderLeft: `4px solid ${C.amber}` }}>
          Votre profil est en attente de validation. Vous pourrez publier des annonces une fois vérifié.
        </div>
      )}
      ...
      {user.description && <p className="sans" style={{ marginTop: 20, lineHeight: 1.7, color: "#20232A", fontSize: 15 }}>{user.description}</p>}
      ...
      <div style={{ marginTop: 32 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <div className="sans" style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.06em", color: "#8A909B" }}>
            {isOwner ? "MES ANNONCES" : "ANNONCES"} ({listings.length})
          </div>
          {isOwner && user.status === "approved" && (
            <button className="btn sans" onClick={() => onNav({ name: "new-listing" })} style={{ background: C.amber, color: "#fff", padding: "8px 14px", borderRadius: 8, fontSize: 12, fontWeight: 700 }}>
              + Publier une annonce
            </button>
          )}
        </div>
        {listings.length === 0 ? (
          <p className="sans" style={{ fontSize: 13, color: "#8A909B" }}>
            {isOwner ? "Aucune annonce publiée pour l'instant." : "Ce prestataire n'a pas encore publié d'annonce."}
          </p>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px,1fr))", gap: 12 }}>
            {sortListings(listings).map((l) => <ListingCard key={l.id} l={l} categories={categories} onNav={onNav} />)}
          </div>
        )}
      </div>
...
function EditProfileForm({ user, categories, onSave, onCancel }) {
  const [form, setForm] = useState({ name: user.name, phone: user.phone, city: user.city, category: user.category, description: user.description || "" });
  ...
  return (
    <section style={{ padding: "30px 0", maxWidth: 520 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <Field label="À propos de vous (optionnel)">
...
function BoostPanel({ item, onRequestBoost, onCancelBoostRequest, flash }) {
  const [picking, setPicking] = useState(false);
  const [copied, setCopied] = useState(false);
  const active = isBoostActive(item);
  const remaining = daysLeft(item);
  const req = item.boostRequest;
    ...
    return (
      <div className="card" style={{ marginTop: 20, padding: 18, borderRadius: 10, borderLeft: `4px solid ${C.amber}` }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
          ...
          <button className="btn sans" onClick={() => onCancelBoostRequest(item)} style={{ background: "transparent", border: `1px solid ${C.line}`, color: "#5A6270", padding: "8px 14px", borderRadius: 8, fontSize: 12 }}>
            Annuler la demande
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        ...
        {!active && (
          <button className="btn sans" onClick={() => setPicking((v) => !v)} style={{ background: C.amber, color: "#fff", padding: "10px 16px", borderRadius: 8, display: "flex", alignItems: "center", gap: 6 }}>
            <Rocket size={14} /> Booster cette annonce
          </button>
        )}
      </div>
        ...
        <div style={{ marginTop: 16, display: "flex", gap: 10, flexWrap: "wrap" }}>
          {BOOST_OFFERS.map((o) => (
            <button
              ...
              className="btn"
              onClick={async () => {
                const ref = await onRequestBoost(item, o.days);
...
function ListingForm({ me, categories, existing, onSave, onNav }) {
  const [form, setForm] = useState({
    title: existing?.title || "",
    category: existing?.category || me?.category || categories[0]?.id || "",
    city: existing?.city || me?.city || "",
  ...
  if (!me) {
    return (
      <section style={{ padding: "30px 0" }}>
        <p className="sans">Connectez-vous pour publier une annonce.</p>
        <button className="btn" onClick={() => onNav({ name: "login" })} style={{ background: C.amber, color: "#fff", padding: "10px 16px", borderRadius: 8, marginTop: 10 }}>Se connecter</button>
      </section>
    );
  }
  if (me.status !== "approved") {
    return (
      <section style={{ padding: "30px 0" }}>
        <p className="sans">Votre profil doit être validé avant de pouvoir publier une annonce.</p>
        <button className="btn" onClick={() => onNav({ name: "profile", id: me.id })} style={{ background: C.amber, color: "#fff", padding: "10px 16px", borderRadius: 8, marginTop: 10 }}>Voir mon profil</button>
      </section>
    );
  }
  ...
  const handlePhotos = async (e) => {
    ...
    if (form.photos.length + files.length > 6) {
      setErr("6 photos maximum par annonce.");
      return;
    }
    setUploading(true);
    try {
      const dataUrls = await Promise.all(files.map((f) => fileToCompressedDataUrl(f, 700, 0.72)));
  ...
  const submit = async () => {
    ...
    if (form.photos.length === 0) {
      setErr("Ajoutez au moins une photo à votre annonce.");
      return;
    }
    const listing = existing
      ? { ...existing, ...form, price: form.price ? Number(form.price) : null }
      : {
          id: uid(),
          userId: me.id,
          userName: me.name,
  ...
  return (
    <section style={{ padding: "30px 0", maxWidth: 560 }}>
      <BackBtn onNav={onNav} label="Retour" />
      <h2 className="disp" style={{ fontSize: 26, margin: "14px 0 20px" }}>{existing ? "Modifier l'annonce" : "Publier une annonce"}</h2>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <Field label={`Photos (${form.photos.length}/6) — au moins 1`}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(90px,1fr))", gap: 8, marginBottom: 8 }}>
            {form.photos.map((src, i) => (
              <div key={i} style={{ position: "relative" }}>
                <img src={src} alt="" style={{ width: "100%", height: 80, objectFit: "cover", borderRadius: 8, border: `1px solid ${C.line}` }} />
                <button className="btn" onClick={() => removePhoto(i)} style={{ position: "absolute", top: -6, right: -6, width: 20, height: 20, borderRadius: "50%", background: C.rust, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>
                  <X size={12} />
                </button>
              </div>
            ))}
          ...
          {form.photos.length < 6 && (
            <label className="btn sans" style={{ background: "#fff", border: `1px solid ${C.line}`, color: C.ink, padding: "9px 14px", borderRadius: 8, display: "inline-flex", alignItems: "center", gap: 6, fontWeight: 600, fontSize: 13 }}>
              <Camera size={15} /> {uploading ? "Chargement…" : "Ajouter des photos"}
              <input type="file" accept="image/*" multiple onChange={handlePhotos} style={{ display: "none" }} />
            </label>
          )}
        </Field>
        <Field label="Titre de l'annonce"><input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Ex. : Installation électrique, rénovation complète" style={inputStyle} /></Field>
        ...
        <Field label="Ville"><input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} style={inputStyle} /></Field>
        <Field label="Prix en FCFA (optionnel)"><input type="number" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="Ex. : 15000" style={inputStyle} /></Field>
        <Field label="Description">
          <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={5} style={{ ...inputStyle, resize: "vertical" }} />
        </Field>
        {err && <p className="sans" style={{ color: C.rust, fontSize: 13, margin: 0 }}>{err}</p>}
        <button className="btn" onClick={submit} style={{ background: C.amber, color: "#fff", padding: "13px 18px", borderRadius: 8, marginTop: 6 }}>
          {existing ? "Enregistrer les modifications" : "Publier l'annonce"}
        </button>
      </div>
    </section>
  );
...
function ListingDetail({ listing, me, categories, onNav, flash, onDelete, onRequestBoost, onCancelBoostRequest, onUpdateRating, onReport }) {
  const [active, setActive] = useState(0);
  const [reviews, setReviews] = useState([]);
  const [reporting, setReporting] = useState(false);
  const reviewsKey = listing ? `reviews:${listing.id}` : null;
  ...
  if (!listing) {
    return (
      <section style={{ padding: "30px 0" }}>
        <BackBtn onNav={onNav} label="Retour" />
        <p className="sans" style={{ marginTop: 20 }}>Annonce introuvable — elle a peut-être été supprimée.</p>
      </section>
    );
  }
  ...
  const submitReview = async (rating, comment) => {
    ...
    await onUpdateRating(listing, Math.round(avg * 10) / 10, next.length);
  ...
  return (
    <section style={{ padding: "30px 0", maxWidth: 640 }}>
      ...
      <div style={{ marginTop: 16, borderRadius: 10, overflow: "hidden", background: C.paperDeep, aspectRatio: "4/3" }}>
        {listing.photos?.[active] && <img src={listing.photos[active]} alt={listing.title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />}
      </div>
      {listing.photos?.length > 1 && (
        <div style={{ display: "flex", gap: 8, marginTop: 8, overflowX: "auto" }}>
          {listing.photos.map((src, i) => (
            <button key={i} className="btn" onClick={() => setActive(i)} style={{ padding: 0, border: i === active ? `2px solid ${C.amber}` : `2px solid transparent`, borderRadius: 8, flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginTop: 18, flexWrap: "wrap", gap: 10 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <h1 className="disp" style={{ fontSize: 22, margin: 0 }}>{listing.title}</h1>
          ...
          {listing.price != null && <div className="disp" style={{ fontSize: 20, color: C.amberDeep, marginTop: 6 }}>{fcfa(listing.price)}</div>}
          <div className="sans" style={{ fontSize: 13, color: "#5A6270", marginTop: 4 }}>{cat?.label} · {listing.city}</div>
          {listing.ratingCount > 0 && <div style={{ marginTop: 6 }}><Stars rating={listing.avgRating} count={listing.ratingCount} size={14} /></div>}
        </div>
        {!isOwner && (
          <button className="btn sans" onClick={() => setReporting(true)} style={{ background: "transparent", color: "#9AA0AA", padding: 0, fontSize: 11, textDecoration: "underline" }}>
      ...
      {reporting && (
        <ReportBox onCancel={() => setReporting(false)} onSubmit={async (reason) => { await onReport(listing.id, listing.title, reason); setReporting(false); }} />
      )}

      <p className="sans" style={{ marginTop: 18, lineHeight: 1.7, color: "#20232A", fontSize: 15, whiteSpace: "pre-wrap" }}>{listing.description}</p>

      <button className="btn sans card" onClick={() => onNav({ name: "profile", id: listing.userId })} style={{ display: "flex", alignItems: "center", gap: 10, padding: 12, borderRadius: 10, marginTop: 18, width: "100%", textAlign: "left" }}>
        <Avatar photo={null} name={listing.userName} size={38} />
        <div>
          <div style={{ fontWeight: 700, fontSize: 14 }}>{listing.userName}</div>
          <div style={{ fontSize: 12, color: "#8A909B", display: "flex", alignItems: "center", gap: 4 }}><ShieldCheck size={12} color={C.stamp} /> Voir le profil</div>
        </div>
      </button>

      {!isOwner && (
        <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap" }}>
          <a href={`tel:${listing.userPhone}`} className="btn sans" style={{ background: C.ink, color: "#fff", padding: "11px 18px", borderRadius: 8, display: "flex", alignItems: "center", gap: 7, textDecoration: "none" }}>
            <Phone size={15} /> Appeler
          </a>
          <a href={`https://wa.me/${listing.userPhone?.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" className="btn sans" style={{ background: C.stamp, color: "#fff", padding: "11px 18px", borderRadius: 8, display: "flex", alignItems: "center", gap: 7, textDecoration: "none" }}>
            <MessageCircle size={15} /> WhatsApp
          </a>
        </div>
      )}
      ...
      {isOwner && (
        <>
          <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
            <button className="btn sans" onClick={() => onNav({ name: "new-listing", editId: listing.id })} style={{ background: "transparent", border: `1px solid ${C.line}`, color: "#5A6270", padding: "9px 14px", borderRadius: 8, fontSize: 13 }}>
              Modifier l'annonce
            </button>
            <button className="btn sans" onClick={() => window.confirm("Supprimer définitivement cette annonce ?") && onDelete(listing)} style={{ background: "transparent", color: C.rust, padding: "9px 14px", borderRadius: 8, fontSize: 13 }}>
              Supprimer
            </button>
          </div>
          <BoostPanel item={listing} onRequestBoost={onRequestBoost} onCancelBoostRequest={onCancelBoostRequest} flash={flash} />
        </>
      )}
...
function Signup({ me, categories, onCreated, onNav }) {
  const [form, setForm] = useState({ name: "", email: "", phone: "", city: "", category: categories[0]?.id || "", description: "", password: "" });
  const [err, setErr] = useState("");
  const [sending, setSending] = useState(false);
  const [sent, setSent] = useState(false);

  const submit = async () => {
    if (!form.name.trim() || !form.email.trim() || !form.phone.trim() || !form.city.trim() || !form.password.trim()) {
      setErr("Merci de remplir tous les champs.");
      return;
    }
    ...
    const u = { id: uid(), ...rest, authId: data.user?.id || null, status: "pending", createdAt: Date.now() };
    await onCreated(u);
    setSending(false);
    setSent(true);
  };
  ...
  if (sent) {
    return (
      <section style={{ padding: "30px 0", maxWidth: 480 }}>
        <div className="card" style={{ padding: 24, borderRadius: 10, textAlign: "center" }}>
          <div style={{ width: 48, height: 48, borderRadius: "50%", background: C.paperDeep, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px" }}>
            <ShieldCheck size={22} color={C.stamp} />
          </div>
          <h2 className="disp" style={{ fontSize: 20, margin: "0 0 8px" }}>Vérifiez votre boîte mail</h2>
          <p className="sans" style={{ fontSize: 14, color: "#5A6270", lineHeight: 1.6 }}>
            Un email de confirmation a été envoyé à <b>{form.email}</b>. Cliquez sur le lien qu'il contient pour activer votre compte, puis connectez-vous. Votre profil sera ensuite examiné avant de pouvoir publier des annonces.
          </p>
          <button className="btn" onClick={() => onNav({ name: "login" })} style={{ background: C.amber, color: "#fff", padding: "11px 20px", borderRadius: 8, marginTop: 16 }}>
  ...
  return (
    <section style={{ padding: "30px 0", maxWidth: 520 }}>
      <BackBtn onNav={onNav} label="Retour" />
      <h2 className="disp" style={{ fontSize: 28, margin: "14px 0 4px" }}>Créer mon compte prestataire</h2>
      <p className="sans" style={{ fontSize: 13, color: "#5A6270", marginBottom: 20 }}>Votre email sera vérifié, puis votre profil validé. Vous pourrez ensuite publier vos annonces avec photos.</p>

      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <Field label="Nom complet"><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} style={inputStyle} /></Field>
        <Field label="Adresse email"><input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="vous@gmail.com" style={inputStyle} /></Field>
        ...
        {err && <p className="sans" style={{ color: C.rust, fontSize: 13, margin: 0 }}>{err}</p>}
        <button className="btn" onClick={submit} disabled={sending} style={{ background: C.amber, color: "#fff", padding: "13px 18px", borderRadius: 8, marginTop: 6 }}>
    ...
    if (error) {
      setErr(
        error.message.includes("Email not confirmed")
          ? "Confirmez d'abord votre email (lien envoyé lors de l'inscription)."
          : "Email ou mot de passe incorrect."
      );
      setSending(false);
      return;
    }
    const u = users.find((x) => x.authId === data.user.id);
    if (!u) {
      setErr("Compte confirmé mais profil introuvable — contactez le support.");
      setSending(false);
      return;
    }
    onLogin(u);
  ...
  return (
    <section style={{ padding: "30px 0", maxWidth: 420 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {err && <p className="sans" style={{ color: C.rust, fontSize: 13, margin: 0 }}>{err}</p>}
        <button className="btn" onClick={submit} disabled={sending} style={{ background: C.ink, color: "#fff", padding: "13px 18px", borderRadius: 8 }}>{sending ? "Connexion…" : "Se connecter"}</button>
        <p className="sans" style={{ fontSize: 13, color: "#5A6270" }}>
...
function Admin({ users, listings, categories, suggestions, reports, boostHistory, onDecision, onAddCategory, onRenameCategory, onDeleteCategory, onSuggestionUpdate, onToggleBoost, onConfirmBoost, onRejectBoostRequest, onReportUpdate, onNav }) {
  const [unlocked, setUnlocked] = useState(false);
  const [code, setCode] = useState("");
  const pending = users.filter((u) => u.status === "pending");
  const decided = users.filter((u) => u.status !== "pending");
  const boostRequests = listings.filter((l) => l.boostRequest);
  ...
  if (!unlocked) {
    return (
      <section style={{ padding: "30px 0", maxWidth: 360 }}>
        ...
        <input type="password" value={code} onChange={(e) => setCode(e.target.value)} placeholder="Code d'accès" style={{ ...inputStyle, width: "100%" }} onKeyDown={(e) => e.key === "Enter" && code === ADMIN_CODE && setUnlocked(true)} />
        <button className="btn" onClick={() => setUnlocked(code === ADMIN_CODE)} style={{ background: C.ink, color: "#fff", padding: "11px 16px", borderRadius: 8, marginTop: 10 }}>Entrer</button>
        <p className="sans" style={{ fontSize: 12, color: "#8A909B", marginTop: 10 }}>Accès réservé.</p>
      </section>
    );
  }
  ...
  return (
    <section style={{ padding: "30px 0" }}>
        ...
        {pending.map((u) => (
          <div key={u.id} className="card" style={{ padding: 14, borderRadius: 10, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
            <div>
              ...
              <p className="sans" style={{ fontSize: 12, color: "#5A6270", margin: "4px 0 0", maxWidth: 480 }}>{u.phone} {u.email ? `· ${u.email}` : ""}</p>
              {u.description && <p className="sans" style={{ fontSize: 12, color: "#5A6270", margin: "4px 0 0", maxWidth: 480 }}>{u.description}</p>}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
      ...
      <ListingsAdminPanel listings={listings} categories={categories} onToggleBoost={onToggleBoost} onNav={onNav} />
...
function ListingsAdminPanel({ listings, categories, onToggleBoost, onNav }) {
  return (
    <div style={{ marginTop: 44 }}>
      <h3 className="disp" style={{ fontSize: 20, margin: "0 0 14px" }}>Annonces publiées ({listings.length})</h3>
      {listings.length === 0 && <p className="sans" style={{ color: "#8A909B", fontSize: 13 }}>Aucune annonce publiée pour l'instant.</p>}
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {sortListings(listings).map((l) => (
          <div key={l.id} className="card" style={{ padding: 12, borderRadius: 10, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
            <button className="btn disp" onClick={() => onNav({ name: "listing", id: l.id })} style={{ background: "transparent", padding: 0, fontSize: 14, textDecoration: "underline", textAlign: "left" }}>
              {l.title} <span className="sans" style={{ fontWeight: 400, color: "#8A909B", fontSize: 12 }}>— {l.userName} · {categories.find(c => c.id === l.category)?.label}</span>
            </button>
            <button className="btn sans" onClick={() => onToggleBoost(l)} style={{ background: isBoostActive(l) ? C.amber : "transparent", border: isBoostActive(l) ? "none" : `1px solid ${C.line}`, color: isBoostActive(l) ? "#fff" : "#5A6270", padding: "6px 12px", borderRadius: 8, fontSize: 12, display: "flex", alignItems: "center", gap: 5, flexShrink: 0 }}>
              <Rocket size={12} /> {isBoostActive(l) ? "Retirer boost" : "Booster (offert)"}
            </button>
          </div>
        ))}
      </div>
    ...
    <div style={{ marginTop: 44 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {requests.map((u) => (
          <div key={u.id} className="card" style={{ padding: 14, borderRadius: 10, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
            <div>
              <div className="disp" style={{ fontSize: 15 }}>{u.title}</div>
              <p className="sans" style={{ fontSize: 12, color: "#5A6270", margin: "4px 0 0" }}>
