...
function Signup({ me, categories, onCreated, onNav }) {
  const [form, setForm] = useState({ name: "", email: "", phone: "", city: "", category: categories[0]?.id || "", description: "", password: "", photo: null });
  const [err, setErr] = useState("");
  const [uploading, setUploading] = useState(false);
  const [sending, setSending] = useState(false);
  const [sent, setSent] = useState(false);
  ...
  const submit = async () => {
    if (!form.name.trim() || !form.email.trim() || !form.phone.trim() || !form.city.trim() || !form.description.trim() || !form.password.trim()) {
      setErr("Merci de remplir tous les champs.");
      return;
    }
    ...
    const u = { id: uid(), ...rest, authId: data.user?.id || null, status: "pending", createdAt: Date.now(), gallery: [] };
    await onCreated(u);
    setSending(false);
    setSent(true);
  };
      ...
      <section style={{ padding: "30px 0", maxWidth: 480 }}>
        <div className="card" style={{ padding: 24, borderRadius: 10, textAlign: "center" }}>
          ...
          <p className="sans" style={{ fontSize: 14, color: "#5A6270", lineHeight: 1.6 }}>
            Un email de confirmation a été envoyé à <b>{form.email}</b>. Cliquez sur le lien qu'il contient pour activer votre compte, puis connectez-vous.
          </p>
  ...
  return (
    <section style={{ padding: "30px 0", maxWidth: 520 }}>
      ...
      <p className="sans" style={{ fontSize: 13, color: "#5A6270", marginBottom: 20 }}>Votre email sera vérifié, puis votre profil validé avant publication.</p>
